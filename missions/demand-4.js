...
function render(){
 ...
 if(t==="intro")m.innerHTML=`<div class="eyebrow">Mission 4 • One More Minute</div><h1>Utility and Consumer Equilibrium</h1><p class="big">This mission turns “utility” from a memorized word into something you can calculate, graph and use in situations you already know from real life.</p><div class="scene"><div class="bigIcon">🚿 → 💧 → 🎮 → 🍟</div><p>Hot showers, cold water, gaming and food add-ons all hide the same idea: the next unit can matter less than the previous one.</p></div>`;
 ...
 else m.innerHTML=`<div class="eyebrow">Mission complete</div><h1>Utility Review</h1><p class="big">You can now recognize diminishing utility in real life, calculate TU and MU, and explain one-good equilibrium as a balance between utility gained and the utility sacrificed by spending money.</p><div class="feedback show good">Next: Spend Your ₹500 — Equi-marginal Utility.</div>`;
 save()
}
...
function renderBalanceGame(){
 ...
 function show(){
   if(i>=mux.length){$("g").innerHTML=`<div class="panel goodbg"><h2>Consumer equilibrium</h2><p>The equilibrium margin is where utility gained equals utility sacrificed.</p></div>`;reward("balanceGame",120);return}
...
function renderBoss(){
 quiz("Demand 4 Comprehensive Practice",[
