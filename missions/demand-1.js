...
function render(){
  ...
  if(s.t==="intro"){
    ...
    <div class="grid2">
      <div><h1>Demand and the Law of Demand</h1><p class="big">Your first job: figure out what customers do when the price changes.</p>
    ...
    m.innerHTML=`<div class="eyebrow">Mission complete</div><h1>Demand Review</h1>
...
function initBoardCoach(){
  ...
  let idx=0,score=0;
  el("main").innerHTML=`<div class="eyebrow">Application</div><h1>Demand: mixed practice</h1><p>Six questions. Same concepts. New situations.</p><div id="bossBox"></div><div id="bossFb" class="feedback"></div>`;
  function show(){
    const q=qs[idx];
    ...
    document.querySelectorAll("[data-b]").forEach(b=>b.addEventListener("click",()=>{
      ...
      if(idx<qs.length){setTimeout(()=>{el("bossFb").className="feedback";show()},380)}
        ...
        el("bossBox").innerHTML=`<div class="panel ${score>=5?"goodbg":"warnbg"}"><h1>${score}/6</h1><p>${score>=5?"Demand review complete.":"Close. Repeat the mixed examples until you can do them without relying on familiar examples."}</p></div>`;
